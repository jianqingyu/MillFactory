...
- (void)setDriP:(NSString *)driP{
    if (driP) {
        _driP = driP;
        if (_driP.length>0&&[_titleStr isEqualToString:@"主   石"]) {
            ...
            if ([_driP containsString:@"."]) {
                ...
                if (_driP.length>range.location+2) {
                    _driP = [_driP substringToIndex:range.location+3];
                }
            }
            self.priceLab.text = [NSString stringWithFormat:@"￥%@",_driP];
        }else{
...
- (void)setTitleStr:(NSString *)titleStr{
    if (titleStr) {
        ...
        if ([_titleStr isEqualToString:@"主   石"]) {
            [self.mainBtn setTitle:@"自带" forState:UIControlStateNormal];
            self.nextBtn.hidden = NO;
            [self.nextBtn addTarget:self action:@selector(nextClick:)
